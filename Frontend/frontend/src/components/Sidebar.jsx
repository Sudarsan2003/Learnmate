...
export default function Sidebar({ activeSessionId, onSelectSession, onNewChat, refreshKey }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 h-full flex-shrink-0 border-r border-[#2DD4BF]/10 bg-[var(--bg)] transition-all duration-300 ease-out md:static md:z-auto ${
          isOpen
        ...
        <div className="flex h-full w-72 max-w-[85vw] flex-col md:w-64">
          <div className="flex items-center justify-between gap-2 px-5 py-5">
            <div className="flex min-w-0 items-center gap-2.5">
              ...
              <h1 className="sb-serif truncate text-lg text-[var(--text)]">
            ...
            <div className="flex flex-shrink-0 items-center gap-1.5">
              ...
              <button
                ...
                className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-md text-[var(--muted)] transition-colors hover:bg-[var(--surface)] hover:text-[var(--text)] md:hidden"
              >
                <X size={16} />
              </button>
            </div>
          ...
          <div className="mt-6 flex-1 overflow-y-auto px-3 pb-4">
            <div className="mb-2 flex items-center gap-2 px-1">
              <p className="font-mono text-[10px] uppercase tracking-[0.15em] text-[var(--dim)]">
                Recent
              </p>
            ...
            {loading ? (
              <p className="px-1 text-xs text-[var(--dim)]">loading…</p>
            ) : sessions.length === 0 ? (
              <p className="px-1 text-xs text-[var(--dim)]">No conversations yet</p>
            ) : (
                  ...
                  return (
                    <button
                      ...
                      className={`sb-item group flex items-center gap-2 rounded-md border-l-2 px-2.5 py-2 text-left text-xs ${
                        isActive
                          ? "border-[#C89B3C] bg-[#C89B3C]/[0.12] text-[var(--text)]"
                          : "border-transparent text-[var(--muted)] hover:bg-[var(--surface)]/70 hover:text-[var(--text)]"
                      }`}
                    >
                      <MessageSquare
                        size={13}
                      ...
                      <Trash2
                        ...
                        className="flex-shrink-0 text-[var(--dim)] opacity-0 transition-opacity hover:text-[#E2725B] group-hover:opacity-100"
                      />
                    </button>
                  );
                })}
